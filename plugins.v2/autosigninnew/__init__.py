...
from app.plugins.autosigninnew.openai import OpenAi


class AutoSignInNew(_PluginBase):
    # 插件名称
    plugin_name = "站点自动签到-新版"
    # 插件描述
    plugin_desc = "自动模拟登录、签到站点。\n基于 thsrite 的自动签到改造而来，详情可参考 https://github.com/thsrite"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "autosigninnew_"
    # 加载顺序
    plugin_order = 0
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._openai_url and self._openai_key:
            ...
            logger.info(f"模型加载完成，{self._openai_model}。")
        else:
        ...
        if self._enabled or self._onlyonce:
            ...
            self._site_schema = ModuleHelper.load('app.plugins.autosigninnew.sites',
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._enabled and self._cron:
            ...
            for trigger in triggers:
                ret_jobs.append({
                    "id": f"AutoSignInNew|{trigger.hour}:{trigger.minute}",
                    "name": "站点自动签到服务",
                    "trigger": "cron",
                    "func": self.sign_in,
                    "kwargs": {
